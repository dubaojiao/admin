...
import com.assistant.util.CookieUtil;
...
public class UserCacheComponent {
     ...
     * @return
     */
    public  boolean setUserCache(LoginUser user, String token){
        /*
         * 1.生成一个32位的token
        ...
        if(url.contains(SysConstant.APP)){
            ...
            return this.appUserCache(user,token);
    ...
    private boolean appUserCache(LoginUser user,String token){
        try {
            ...
            CookieUtil.addCookie(response,token,expire7);
    ...
    public LoginUser getUserCache(){
        ...
        if(loginUser != null){
            //TODO 用户信息更新
            //UserInfo userInfo = userDao.findById(loginUser.getUid());
            //loginUser.setUserName(CheckUtil.stringIsNull(userInfo.getName())?userInfo.getNickname():userInfo.getName());
        }
        return loginUser;
    }
    ...
    private LoginUser getWebUserCache(){
        try {
            ...
            //String key = request.getHeader("k");
